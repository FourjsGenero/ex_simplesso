...
MAIN
    ...
    WHILE TRUE
        TRY
            LET req =
                com.WebServiceEngine.GetHttpServiceRequest(C_CLEANUP_DELAY)
            IF req IS NULL THEN
                CALL Cleanup()
            ELSE
                ...
                ELSE
                    ...
                    LET operation =
                        path.subString(
                            ind + C_BASE_PATH.getLength(), path.getLength())

                    #
                    # Rebuild "real" base URL taking HTTPS into account
                    #
                    LET HTTPSON =
                        req.getRequestHeader(HTTPHelper.C_X_FOURJS_HTTPS)
...
FUNCTION DispatchService(req, baseURL, operation)
    ...
    ELSE
        ...
        CASE operation
            ...
            OTHERWISE
                CALL req.sendTextResponse(
                    501, NULL, "operation not implemented")
...
#+
PRIVATE FUNCTION DelegateAuthenticate(req, baseURL, query, originURL)
    DEFINE req com.HttpServiceRequest
    ...
    IF cookieValue IS NULL THEN
        ...
        CALL HTTPHelper.SendWelcomePage(
            req,
            baseURL
                || C_BASE_PATH
                || HTTPHelper.C_VALIDATE
                || "?back="
                || util.Strings.urlEncode(originURL))
    ...
    ELSE
        ...
        IF ok THEN # Cookie valid, user authenticated
            ...
            CALL req.setResponseHeader(
                C_X_FOURJS_FGL_AUTO_LOGOUT_PROMPT_QUERY, p_user)
...
#+ @param originURL initial URL that triggered the delegate (/ua/r/app)
#+
PRIVATE FUNCTION DelegateResumeApp(req, baseURL, query, originURL)
    DEFINE req com.HttpServiceRequest
    DEFINE baseURL STRING # Delegate service URL
    DEFINE query WSHelper.WSQueryType
    DEFINE originURL STRING
...
FUNCTION DoDelegate(req, baseURL, query)
    ...
    IF query.getLength() == 0 THEN
        CALL req.sendTextResponse(
            400, "Bad request", "Delegate query string is missing")
    ...
    IF ind <= 0 THEN
        CALL req.sendTextResponse(
            400, "Bad request", "Delegate query URL is missing")
...
#+
PRIVATE FUNCTION ValidateUser(req, baseURL, query, backURL)
    DEFINE req com.HttpServiceRequest
    DEFINE baseURL STRING
    DEFINE query WSHelper.WSQueryType
    ...
    CALL parseHTMLFormData(
        req.readFormEncodedRequest(TRUE))
        RETURNING p_user, p_pwd, p_connected
    ...
    IF SSOUserFunctions.checkAuth(p_user, p_pwd) THEN
        ...
        IF p_connected THEN
            LET cookies =
                SSOUserFunctions.createSession(
                    p_user, CURRENT + COOKIE_KEEP, backURL)
        ELSE
            LET cookies =
                SSOUserFunctions.createSession(
                    p_user, CURRENT + COOKIE_VALIDITY, backURL)
        END IF
    ...
    # Return error page
    CALL HTTPHelper.sendErrorPage(
        req,
        baseURL
            || C_BASE_PATH
            || HTTPHelper.C_VALIDATE
            || "?back="
            || util.strings.urlEncode(backUrl))
...
#+
PRIVATE FUNCTION ReValidateUser(req, baseURL, query, relayId)
    DEFINE req com.HttpServiceRequest
    DEFINE baseURL STRING
    DEFINE query WSHelper.WSQueryType
    ...
    CALL HTTPHelper.parseHTMLFormData(req.readFormEncodedRequest(TRUE))
        RETURNING p_user, p_pwd, p_connected
    ...
    IF SSOUserFunctions.checkAuth(p_user, p_pwd) THEN
        ...
        IF ua_session IS NOT NULL THEN
            ...
            CALL req.setResponseHeader(
                HTTPHelper.C_HTTP_LOCATION,
                baseURL
                    || HTTPHelper.C_RESUME_URL
                    || ua_session
                    || "?relaystate="
                    || relayId)
            CALL req.sendResponse(302, NULL)
...
FUNCTION DoPrompt(req, baseURL, query)
    ...
    IF query.getLength() == 0 THEN
        CALL req.sendTextResponse(
            400, "Bad Request", "Prompt query string is missing")
    ...
    IF ind == 0 THEN
        CALL req.sendTextResponse(
            400, "Bad Request", "Prompt session is missing")
    ...
    IF ind == 0 THEN
        CALL req.sendTextResponse(
            400, "Bad Request", "Prompt timeout is missing")
    ...
    LET uuid =
        SSOUserFunctions.createRelayState(
            prompt_id, session_id, CURRENT + RELOGIN_TIME)

    # Start reauthentication page and send reponse to /Validate?relaystate operation
    CALL HTTPHelper.SendRelogPage(
        req,
        baseURL
            || C_BASE_PATH
            || HTTPHelper.C_VALIDATE
